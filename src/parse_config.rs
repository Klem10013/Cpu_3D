...
pub fn make_plane(pos : (f64,f64,f64), size : (f64,f64,f64), color : u32, only_face : bool, inverse_normal : bool) -> (Triangle3D,Triangle3D){
...
pub fn generate_config() -> (Camera,Vec<Triangle3D>){
    ...
    for rec in &config.rectangles {
        //top face
        let (t1,t2) = make_plane(rec.x,(rec.size.0,0f64,rec.size.2),rec.color,rec.only_face,rec.inverse_normal);
        triangles.push(t1);
        triangles.push(t2);

        //front face
        let (t1,t2) = make_plane(rec.x,(rec.size.0,rec.size.1,0f64),rec.color,rec.only_face,rec.inverse_normal);
        triangles.push(t1);
        triangles.push(t2);

        //left face
        let (t1,t2) = make_plane(rec.x,(0f64,rec.size.1,rec.size.2),rec.color,rec.only_face,!rec.inverse_normal);
        triangles.push(t1);
        triangles.push(t2);

        //right face
        let (t1,t2) = make_plane((rec.x.0 + rec.size.0,rec.x.1,rec.x.2),(0f64,rec.size.1,rec.size.2),rec.color,rec.only_face,rec.inverse_normal);
        triangles.push(t1);
        triangles.push(t2);

        //back face
        let (t1,t2) = make_plane((rec.x.0,rec.x.1,rec.x.2 + rec.size.2),(rec.size.0,rec.size.1,0f64),rec.color,rec.only_face,!rec.inverse_normal);
        triangles.push(t1);
        triangles.push(t2);

        //bottom face
        let (t1,t2) = make_plane((rec.x.0,rec.x.1 + rec.size.1,rec.x.2),(rec.size.0,0f64,rec.size.2),rec.color,rec.only_face,!rec.inverse_normal);
        triangles.push(t1);
        triangles.push(t2);
    }
